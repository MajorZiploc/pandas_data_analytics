...
function get_file_changes () { echo "lines_added,lines_deleted,file_name,author"; authors=(`git --no-pager shortlog -sn --all | col_n 2 | xargs`);for author in ${authors[@]}; do git --no-pager log --author="$author" --format=tformat: --numstat --after "11/12/21" --before "11/20/21" | sed -E "s/[[:blank:]]+/,/g;s/(.*)/\1,$author/g"; done; }
